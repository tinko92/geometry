...
// Copyright (c) 2007-2018 Barend Gehrels, Amsterdam, the Netherlands.
// Copyright (c) 2013-2018 Adam Wulkiewicz, Lodz, Poland.
...
struct side_by_generic_form
{
    ...
    static inline int apply(P1 const& p1, P2 const& p2, P const& p)
    {
        ...
        double dist = form.distance_measure(get<0>(p), get<1>(p));
    ...
    template <typename P1, typename P2, typename P>
    static inline double distance_measure(P1 const& p1, P2 const& p2, P const& p)
    {
        arithmetic::general_form<double> form = arithmetic::construct_line<double>(p1, p2);
        return form.distance_measure(get<0>(p), get<1>(p));
    }
...
//    {
//        return arithmetic::is_zero(value, policy);
//    }
    ...
    static inline typename geometry::coordinate_type<Point>::type
            distance_measure(Point const& p, Segment const& s)
    {
    ...
    distance_measure_segseg(Segment1 const& a, Segment2 const& b)
    {
    ...
    <
     ...
     >
    static inline
    void initialize_distance_measures(Data& data,
               Segment1 const& a, Segment2 const& b,
               arithmetic::general_form<T> const& gfa,
               arithmetic::general_form<T> const& gfb)
    {
        data[0].value = gfb.distance_measure(geometry::get<0, 0>(a), geometry::get<0, 1>(a)); // a[0] w.r.t. b
        data[1].value = gfb.distance_measure(geometry::get<1, 0>(a), geometry::get<1, 1>(a)); // a[1] w.r.t. b
        data[2].value = gfa.distance_measure(geometry::get<0, 0>(b), geometry::get<0, 1>(b)); // b[0] w.r.t. a
        data[3].value = gfa.distance_measure(geometry::get<1, 0>(b), geometry::get<1, 1>(b)); // b[1] w.r.t. a
    ...
    {
        typedef typename geometry::coordinate_type<Segment1>::type ct;
        double d = distance_measure_segseg<Index1, Index2>(a, b);
                    ...
                    double& fraction, bool& doubt)
    {
        ...
        // If fraction is -0.0001, then result is false. But it might be
        // a numerical error. In that case, try to verify in other ways too
        //in plaats hiervan: kijk of het * 1000 afgerond 0 of 1000 is.
        //boost::math::iround oid.
        //Dat kan later gebruikt worden om 0/1 index 0/1 van a/b te vergelijken, dan
        //is er daar maar 1 vergelijking nodig.
        //Plus, als het gelijk is, wordt de fractie dan precies 0 of 1.
        ...
        {
            double const d
                = near_zero ? distance_measure<0>(point, segment)
                : near_one ? distance_measure<1>(point, segment)
                : 1.0;
                    ...
                    double& fraction, bool& doubt)
    {
        ...
        // y or x direction
        bool const horizontalish = f.more_horizontal();
        ...
        if (! result && doubt)
        {
            ...
            if (horizontalish && f.has_vertical_component())
            {
            ...
            if (! horizontalish && f.has_horizontal_component())
            {
                               ...
                               double& fraction_a, double& fraction_b)
    {
        ...
        dist[0] = distance_measure_segseg<0, 0>(a, b);
        dist[1] = distance_measure_segseg<0, 1>(a, b);
        dist[2] = distance_measure_segseg<1, 0>(a, b);
        dist[3] = distance_measure_segseg<1, 1>(a, b);
    ...
    bool disjoint_by_coordinates(Segment1 const& a, Segment2 const& b,
                      arithmetic::general_form<T> const& f)
    {
        return (f.has_horizontal_component() && disjoint_in_dimension<0>(a, b))
            || (f.has_vertical_component()   && disjoint_in_dimension<1>(a, b));
    }
                       ...
                       arithmetic::general_form<T> const& gf_b)
    {
        ...
        double const dm_a0 = gf_b.distance_measure(geometry::get<0, 0>(a), geometry::get<0, 1>(a)); // a[0]
        double const dm_a1 = gf_b.distance_measure(geometry::get<1, 0>(a), geometry::get<1, 1>(a)); // a[1]
        ...
        double const dm_b0 = gf_a.distance_measure(geometry::get<0, 0>(b), geometry::get<0, 1>(b)); // b[0]
        double const dm_b1 = gf_a.distance_measure(geometry::get<1, 0>(b), geometry::get<1, 1>(b)); // b[1]
        ...
        if (consider_as_crossing)
        {
            ...
            std::cout << "consider as crossing" << std::endl;

            // It still can be considered as collinear too
        }
        bool consider_as_touching = ! consider_as_crossing && (side_a < 0 || side_b < 0);
        if (consider_as_touching)
        {
            std::cout << "consider as touching" << std::endl;
        }
        ...
        consider_as_disjoint = dm_separation > 1.0e-12; // TODO - the 'magic' distance
                                  ...
                                  double& fraction_a, double& fraction_b)
    {
        ...
        if (on_a && doubt_b)
        {
            ...
            if (fb == 0)
            {
                double const value = gfa.distance_measure(geometry::get<0, 0>(b), geometry::get<0, 1>(b)); // b[0] vs a
            ...
            if (fb == 1)
            {
                double const value = gfa.distance_measure(geometry::get<1, 0>(b), geometry::get<1, 1>(b)); // b[1] vs a
                if (std::fabs(value) < general_distance_threshold<ct>::get())
                {
                    fraction_b = 1.0;
                    return true;
        ...
        if (on_b && doubt_a)
        {
            ...
            if (fa == 0)
            {
                double const value = gfb.distance_measure(geometry::get<0, 0>(a), geometry::get<0, 1>(a)); // a[0] vs b
                if (std::fabs(value) < general_distance_threshold<ct>::get())
                {
                    fraction_a = 0.0;
                    return true;
            ...
            if (fa == 1)
            {
                double const value = gfb.distance_measure(geometry::get<1, 0>(a), geometry::get<1, 1>(a)); // a[1] vs b
                if (std::fabs(value) < general_distance_threshold<ct>::get())
                {
                    fraction_a = 1.0;
                    return true;
    ...
    struct side_assorter
    {
        ...
        bool is_consistent(Data const& data)
        {
            ...
            for (int i = 0; i < 4; i += 2)
            {
                if (data[i].side != 0 && data[i + 1].side != 0)
                {
                    if (data[i].side != -data[i + 1].side)
                    {
                        ...
                        std::cout << "Inconsistent side " << g_case_id << std::endl;
                        return false;
                    }
                }
            }
            ...
            {
                std::cout << "Too many collinearities " << g_case_id << std::endl;
                return false;
            }
...
//er moet een is_touching komen die 1 side<=0 af kan handelen, en dan
//fractie zet, en dan is dit situatie consistent.

            double const eps = 0.01; // TODO
            double const eps_f = 0.1; // TODO

            // There are 8 posibilities
            if (index == 0 && wrong_index == 2
                    && std::fabs(fb) < eps_f
                    && distance_measure_segseg<0, 0>(a, b) < eps)
            {
                fb = 0.0;
            ...
            if (index == 0 && wrong_index == 3
                    ...
                    && distance_measure_segseg<0, 1>(a, b) < eps)
            {
                fb = 1.0;
                data[wrong_index].on_end = true;
                return true;
            ...
            if (index == 1 && wrong_index == 2
                    && std::fabs(fb) < eps_f
                    && distance_measure_segseg<1, 0>(a, b) < eps)
            {
                fb = 0.0;
                data[wrong_index].on_end = true;
                return true;
            ...
            if (index == 1 && wrong_index == 3
                    && std::fabs(fb - 1.0) < eps_f
                    && distance_measure_segseg<1, 1>(a, b) < eps)
            {
                fb = 1.0;
                data[wrong_index].on_end = true;
                return true;
                   ...
                   arithmetic::general_form<T> const& gf_b)
        {
            ...
            if (!consistent)
            {
                ...
                {
                    std::cout << "Inconsistent threshold " << g_case_id << " consider as touching" << std::endl;
                    initialize(data, a, b, gf_a, gf_b, fa, fb);
                ...
                if (!consistent)
                {
                    ...
                    {
                        std::cout << "Inconsistent threshold " << g_case_id << " -> consider as collinear" << std::endl;
                        return false;
                    }
                    ...
                    std::cout << "Inconsistent case " << g_case_id << " -> REMEDY UNKNOWN" << std::endl;
                    return false;
                }
                            ...
                            RobustPoint2 const& robust_b1, RobustPoint2 const& robust_b2)
    {
        ...
        const double value_a = fai == 0
            ? gfb.distance_measure(geometry::get<1, 0>(a), geometry::get<1, 1>(a))
            : gfb.distance_measure(geometry::get<0, 0>(a), geometry::get<0, 1>(a));
        const double value_b = fbi == 0
            ? gfa.distance_measure(geometry::get<1, 0>(b), geometry::get<1, 1>(b))
            : gfa.distance_measure(geometry::get<0, 0>(b), geometry::get<0, 1>(b));
        ...
        const double product = value_a * value_b;

        std::cout << "Verify endpoint crossing " << product << std::endl;
...
#if 0
        bool const a_is_point = segment_equals<0, 1>(a, a, equal_policy);
        bool const b_is_point = segment_equals<0, 1>(b, b, equal_policy);
        if (a_is_point && b_is_point)
        {
            return segment_equals<0, 0>(a, b, equal_policy)
                ? Policy::degenerate(a, true)
                : Policy::disjoint()
                ;
        }
...
#define HANDLE_DOUBT
        ...
        if (! crossing && intersection_doubt)
        {
            ...
            initialize_distance_measures(data, a, b, gf_a, gf_b);
            ...
            // They can be collinear and still cross
            if (! consider_as_collinear)
            {
                bool consistent = true;
                side_info sides;

                double fa = -1, fb = -1; // TODO: how do we know?
        ...
        if (crossing)
        {
            ...
            if (get_fractions_on_segment(a, b, first_result, gf_a, gf_b, fa, fb))
            {
                ...
                int fa_i = -1, fb_i = -1;
                bool const susp = suspicious(fa_i, fb_i, fa, fb);
...
//                    if (counter == 2014)
                ...
                //                // TODO: if rescaling is gone, this can go too.
//                long long int const denominator = 9999999999999; // 1000000;
//                                               9223372036854775807
//                sinfo.robust_ra.assign(fa * denominator, denominator);
//                sinfo.robust_rb.assign(fb * denominator, denominator);
...
//                std::cout << "Outside segment #" << counter << " " << fa << " " << fb << std::endl;
                return Policy::disjoint();
            }
        }
        ...
        if (intersection_doubt)
        {
            ...
            consistent = side_assorter<true>::apply(sides, fa, fb,
                                                    consider_as_collinear,
                                                    a, b,
                                       gf_a, gf_b, equal_policy);

            evaluate_endpoints(a, b, equal_policy, fa, fb);
        }
#endif
        ...
        arithmetic::general_form<double> gf_norm_a = gf_a.template normalize<double>();
        arithmetic::general_form<double> gf_norm_b = gf_b.template normalize<double>();

        if (consider_as_collinear
            || lines_collinear(gf_norm_a, gf_norm_b, equal_policy))
        {
//            std::cout << "COLLINEAR Counter: " << counter << std::endl;
            // Lines are collinear. There are 0, 1 or 2 intersections
            return gf_a.more_horizontal()
