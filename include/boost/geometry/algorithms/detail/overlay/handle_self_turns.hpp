...
private :
            ...
            const Turns& turns, Clusters const& clusters)
    {
        ...
        {
            if (! is_self_turn<OverlayType>(turns[tid]))
            {
                return false;
            }
        }
            ...
            Strategy const& strategy)
    {
        ...
        {
            auto const cluster_id = cluster.first;
                ...
                && is_self_cluster(cluster_id, turns, clusters))
            {
                ...
                auto const index = *cinfo.turn_indices.begin();
                if (! check_within<OverlayType>::apply(turns[index],
                                                       geometry0, geometry1,
                                                       strategy))
                {
                    // Discard all turns in cluster
                    for (auto const& s : cinfo.turn_indices)
                    {
                        turns[s].discarded = true;
                    }
                }
            }
        }
