...
struct get_relative_order
{
    ...
    {
        auto const side_ti_u = strategy.apply(ti, tj, ui);
        auto const side_tj_u = strategy.apply(ti, tj, uj);

#ifdef BOOST_GEOMETRY_DEBUG_RELATIVE_ORDER
        std::cout << (factor == 1  ? " r//s " :  " s//r ")
            << static_cast<int>(side_ti_u) << " / " << static_cast<int>(side_tj_u);
#endif

        return static_cast<int>(side_ti_u * side_tj_u) >= 0
            ? factor * static_cast<int>(side_ti_u != side_type::collinear ? side_ti_u : side_tj_u)
            : 0;
    }
    ...
    static inline int apply(
                ...
                SideStrategy const& strategy)
    {
        auto const side_ri_p = strategy.apply(pi, pj, ri);
        auto const side_si_p = strategy.apply(pi, pj, si);

#ifdef BOOST_GEOMETRY_DEBUG_RELATIVE_ORDER
        auto const side_rj_p = strategy::apply(pi, pj, rj);
        auto const side_sj_p = strategy::apply(pi, pj, sj);
        std::cout << "r//p: " << static_cast<int>(side_ri_p) << " / " << static_cast<int>(side_rj_p);
        std::cout << " s//p: " << static_cast<int>(side_si_p) << " / " << static_cast<int>(side_sj_p);
#endif
        ...
        int const order = static_cast<int>(side_ri_p * side_ri_p * side_si_p) * value;
